...
def ourCalibrateCamera(img_names,square_size):
    pattern_size = (8, 6)
...
def our_calibration(exif,data):
    focal = exif['focal_ratio']
    mtx,dist=ourCalibrateCamera(data.imagesCalib(),data.config['square_size'])
    #focal=cv2.calibrationMatrixValues(mtx, imageSize, apertureWidth, apertureHeight)
    k1=dist[0][0]
    k2=dist[0][1]
    focal=0.8055555555555556*(4.4/4.3)
    
    return {'focal':focal ,'k1':k1,'k2':k2}
...
def camera_from_exif_metadata(metadata, data):
    ...
    if pt == 'perspective':
        calib = (hard_coded_calibration(metadata)
                 or our_calibration(metadata,data)
